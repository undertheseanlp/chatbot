...
static FunctionResult RestoreSentenceCode(char* buffer)
{
	...
	for (int i = 1; i <= wordCount; ++i)
	{
		...
		finalPosValues[i] <<= 32;
		finalPosValues[i] |= memory[n++];
		concepts[i] = memory[n++]; // contain heap space indices that are still valid
		topics[i] = memory[n++];
	}
    ...
    *buffer = 0;
    ...
    if (*scan == '"') // if a quoted string as burst char, remove the quotes
    {
        ...
        if (*scan == 0) // burst into characters
        {
            ...
            while (*++ptr && ptr[1]) // leave rest for end
            {
                ...
                word[charLen] = 0;
                ptr += charLen - 1;
                if (tracing) Log(STDTRACELOG, (char*)"[%d]: %s\r\n", counter, word);
                ++counter;
    ...
    while (hold)
    {
        *hold = 0;	//   terminate first piece
        ...
        if (tracing)
        {
            if (impliedWild != ALREADY_HANDLED) Log(STDTRACELOG, (char*)"counter: %d _%d= %s\r\n", counter, wildcardIndex - 1, ptr);
            else Log(STDTRACELOG, (char*)"counter: %d %s\r\n", counter, ptr);
        }
    ...
    if (tracing)
    {
        if (impliedWild != ALREADY_HANDLED) Log(STDTRACELOG, (char*)"counter %d: _%d= %s ", counter, wildcardIndex - 2, ptr);
        else Log(STDTRACELOG, (char*)"counter %d: %s ", counter, ptr);
    }
...
// e.g.  TCPOpen(POST "http://de.sempar.ims.uni-stuttgart.de/parse" "sentence=ich+bin&returnType=rdf" 'myfunc)
	...
	uint64 start_time = ElapsedMilliseconds();
	int size = 0;
	char* tcpbuffer = AllocateStack(NULL,INPUT_BUFFER_SIZE);
	char* startContent = tcpbuffer;
	try 
	{
		if (trace & TRACE_TCP && CheckTopicTrace()) Log(STDTRACELOG,(char*)"RAW TCP: %s/%s port:%d %s %s",url,directory, portid,(kind == 'G' ) ? (char*)"GET" : (char*) "POST",originalArg);
	...
	char* buf1 = AllocateStack(NULL,INPUT_BUFFER_SIZE);
	buf1[0] = '(';
	buf1[1] = ' ';
